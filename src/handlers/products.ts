...
const update = async (_req: Request, res: Response): Promise<void> => {
  try {
     ...
     * so if the user didn't provide any of them will be acceptable
     */
    Object.keys(product).forEach((key: string): boolean=> product[key as ProductKeyType] === undefined && delete product[key as ProductKeyType]); 
    ...
    else if (product.price && (+product.price < 0 || Number.isNaN(+product.price))) {
      res.status(400);
      res.json('The price should be a positive number');
    } else {
...
const destroy = async (_req: Request, res: Response): Promise<void> => {
  ...
  res.json(deletedProduct || `Product with id ${_req.params.id} not found to be deleted.`);
}
